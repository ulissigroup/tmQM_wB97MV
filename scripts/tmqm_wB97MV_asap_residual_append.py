...
#read the trajectory file containing the tmqm_wB97MV data
traj = Trajectory('../all_data/tmQM_wB97MV/tmQM_wB97MV.traj')

#read the predictions npz from the model
dataset = LmdbDataset({"src":"../training_data/tmQM_wB97MV/80-test.lmdb"})
...
def append_predictions(predictions, test):
    ...
    for j in trange(len(test)):
        ...
        data.residual = (data.y_relaxed - data.predicted) * Hartree #in eV
...
new_dataset = combine_datasets(traj, pred_dataset)

print('Writing files')
write('../predictions/tmQM_wB97MV/80-gemnet_evpredictions.traj', new_dataset, format='traj')